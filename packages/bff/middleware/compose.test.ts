...
function createMockOAuthClient(): AtprotoOAuthClient {
  return {
    restore: (did: string) => {
      // Mock OAuth session restoration
          ...
          com: {
            atproto: {
              server: {
                getSession: () =>
                  Promise.resolve({
                    data: { did, handle: `test.bsky.social` },
                  }),
              },
            },
          },
...
Deno.test("composeMiddlewares - context methods and properties", async () => {
  ...
  const innerHandler = (_req: Request, ctx: BffContext) => {
    ...
    assertExists(ctx.oauthClient);
    assertExists(ctx.oauthClientNative);
    assertExists(ctx.indexService);
