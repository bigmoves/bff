...
Deno.test("IndexService - auth token operations", () => {
  const db = createTestDatabase();
  const cfg = createTestConfig();
  const service = new IndexService(db, cfg);

  // First insert an actor
  const testActor: ActorTable = {
    did: "did:plc:test",
  ...
  service.insertAuthToken(testActor.did, refreshToken, issuedAt, expiresAt);

  const actor = service.getActorByRefreshToken(refreshToken);
  assertExists(actor);
  assertEquals(actor.did, testActor.did);
  assertEquals(actor.handle, testActor.handle);

  service.deleteAuthToken(testActor.did);

  const deletedActor = service.getActorByRefreshToken(refreshToken);
  assertEquals(deletedActor, undefined);
});
